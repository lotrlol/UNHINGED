...
export function ProjectCommentSection({ 
  projectId, 
  className = '', 
  showPreview = false, 
  ...
  const { user } = useAuth()
  const { comments, loading, submitting, createComment, likeComment, deleteComment } = useProjectComments(projectId)
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    const { error } = await createComment(newComment.trim(), null)
  ...
  const handleSubmitReply = async (e: React.FormEvent, parentId: string) => {
    ...
    const { error } = await createComment(replyContent.trim(), parentId)
      ...
      {loading ? (
        ...
        !showPreview && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 text-sm">Be the first to share your thoughts on this project!</p>
          </div>
        )
      ) : (
